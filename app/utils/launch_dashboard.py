from app.train_model import predict_horse_price
from data.data_gathering_script import get_summary_horse_data
from app.utils.get_intrinsic_value import (
    get_intrinsic_value,
    get_intrinsic_value_no_api,
)
from app.train_model import predict_horse_price
...
def launch_dashboard(id):
    ...
    breed_daily = (
        market_data_no_outliers.groupby(["day_sold", "breed_type"]).mean().reset_index()
    )
    ...
    def display_df():
        abc = market_data_no_outliers.loc[
            (market_data_no_outliers["breed_type"] == horse.breed)
            & (market_data_no_outliers["bloodline"] == horse.bloodline)
        ]
        price_10p, price_25p, price_50p, price_75p, price_90p = np.percentile(
            abc["converted_price"], [10, 25, 50, 75, 90]
        )
        (
            win_rate_10p,
            win_rate_25p,
            ...
            win_rate_75p,
            win_rate_90p,
        ) = np.percentile(abc["win_rate"], [10, 25, 50, 75, 90])
        races_10p, races_25p, races_50p, races_75p, races_90p = np.percentile(
            abc["overall.races"], [10, 25, 50, 75, 90]
        )
        (
            totalETH_10p,
            totalETH_25p,
            ...
            totalETH_75p,
            totalETH_90p,
        ) = np.percentile(abc["total_paid"], [10, 25, 50, 75, 90])
        avg_win_rate, avg_races, avg_winnings, avg_price = (
            abc["win_rate"].mean(),
            abc["overall.races"].mean(),
            abc["total_paid"].mean(),
            abc["converted_price"].mean(),
        )
        ...
        ten_p_values = [
            win_rate_formatted[0],
            races_formatted[0],
            totalETH_formatted[0],
            price_formatted[0],
        ]
        twenty_five_p_values = [
            win_rate_formatted[1],
            races_formatted[1],
            totalETH_formatted[1],
            price_formatted[1],
        ]
        median_values = [
            win_rate_formatted[2],
            races_formatted[2],
            totalETH_formatted[2],
            price_formatted[2],
        ]
        seven_five_p_values = [
            win_rate_formatted[3],
            races_formatted[3],
            totalETH_formatted[3],
            price_formatted[3],
        ]
        nine_zero_p_values = [
            win_rate_formatted[4],
            races_formatted[4],
            totalETH_formatted[4],
            price_formatted[4],
        ]
        avg_values = [
            f"{'{0:.2f}'.format(avg_win_rate)}%",
            int(avg_races),
            f"{'{0:.4f}'.format(avg_winnings)} ETH",
            f"${'{0:.2f}'.format(avg_price)}",
        ]
        ...
        data = [
            ...
            [
                twenty_five_p_name,
                twenty_five_p_values[0],
                twenty_five_p_values[1],
                twenty_five_p_values[2],
                twenty_five_p_values[3],
            ],
            [
                median_name,
                median_values[0],
            ...
            [average_name, avg_values[0], avg_values[1], avg_values[2], avg_values[3]],
            [
                seven_five_p_name,
                seven_five_p_values[0],
                seven_five_p_values[1],
                seven_five_p_values[2],
                seven_five_p_values[3],
            ],
            [
                nine_zero_p_name,
                nine_zero_p_values[0],
                nine_zero_p_values[1],
                nine_zero_p_values[2],
                nine_zero_p_values[3],
            ],
        ]
        xyz = pd.DataFrame(
            data,
    ...
    def win_rate_by_breed():
        ...
        sns.barplot(
            ...
            x="breed_type",
            y="win_rate",
            order=["genesis", "legendary", "exclusive", "elite", "cross", "pacer"],
        )
        plt.xlabel("Breed")
        plt.ylabel("Win Rate (%)")
    ...
    def hv_line_breed():
        ...
        list_of_plots = [
            hv.Curve(
                breed_daily.loc[breed_daily["breed_type"] == "genesis"][
                    ["day_sold", "converted_price"]
                ],
                label="Genesis",
            ),
            hv.Curve(
                breed_daily.loc[breed_daily["breed_type"] == "legendary"][
                    ["day_sold", "converted_price"]
                ],
                label="Legendary",
            ),
            hv.Curve(
                breed_daily.loc[breed_daily["breed_type"] == "exclusive"][
                    ["day_sold", "converted_price"]
                ],
                label="Exclusive",
            ),
            hv.Curve(
                breed_daily.loc[breed_daily["breed_type"] == "elite"][
                    ["day_sold", "converted_price"]
                ],
                label="Elite",
            ),
            hv.Curve(
                breed_daily.loc[breed_daily["breed_type"] == "cross"][
                    ["day_sold", "converted_price"]
                ],
                label="Cross",
            ),
            hv.Curve(
                breed_daily.loc[breed_daily["breed_type"] == "pacer"][
                    ["day_sold", "converted_price"]
                ],
                label="Pacer",
            ),
        ]
        return hv.Overlay(list_of_plots).opts(
            xlabel="Date",
            ylabel="Sale Price (USD)",
            title="Sale Price by Breed Over Time",
    ...
    def line_breed():
        ...
        sns.lineplot(
            ...
            y="converted_price",
            hue="breed_type",
            hue_order=["genesis", "legendary", "exclusive", "elite", "cross", "pacer"],
        )
        ...
        plt.legend(
            title="Breed",
            handles=handles,
            labels=["Genesis", "Legendary", "Exclusive", "Elite", "Cross", "Pacer"],
        )
    ...
    def barchart_median_win_by_blood():
        ...
        sns.barplot(
            ...
            x="bloodline",
            y="win_rate",
            order=["Nakamoto", "Szabo", "Finney", "Buterin"],
        )
    ...
    def line_blood():
        ...
        sns.lineplot(
            ...
            y="converted_price",
            hue="bloodline",
            hue_order=["Nakamoto", "Szabo", "Finney", "Buterin"],
        )
        plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True, prune="both"))
        plt.xticks(rotation=20)
        plt.xlabel("Date")
    ...
    template = pn.template.FastListTemplate(
        title="ThoroughZED Analytics",
        logo="https://i.imgur.com/3rpZHfT.png",
        header_background="#09a59f",
        header_color="black",
        font="times",
        shadow=True,
        corner_radius=20,
        favicon="https://i.imgur.com/3rpZHfT.png",
        theme_toggle=False,
        busy_indicator=None,
        sidebar=[
        ...
        main=[
            ...
            pn.Row(pn.Column(hv_line_breed), pn.Column(win_rate_by_breed)),
            pn.Row(pn.Column(line_blood), pn.Column(avg_win_by_bloodline)),
        ],
