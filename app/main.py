...
color_words = [
    colored("Horse/NFT ID", "green", attrs=["bold", "underline"]),
    ...
    colored(
        "> *** This horse has not been raced and cannot have a valuation run on its performance ***",
        "red",
        attrs=["bold"],
    ),
    ...
    colored(
        "> *** Type 'CTRL + C' to exit the online dashboard. ***", "red", attrs=["bold"]
    ),
...
def check_id(id):
    ...
    while not id.isdigit():
        ...
        if id.lower() == "h":
            ...
            print(f"\n{color_words[2]}\n")
        id = input(
            f"> Please enter the {color_words[0]} for the horse you'd like to evaluate. Or type {color_words[1]} to quit or {color_words[5]} for help. "
        )
    return id
...
def check_valid_horse(id):
    ...
    try:
        ...
        if horse_object["data.horse.race_statistic.number_of_races"][0] == 0:
            print(f"\n{color_words[6]}\n")
            answer = input(
                f"> Would you like to display the dashboard for this horse? Type {color_words[8]} for yes or {color_words[9]} for no. "
            )
...
def check_choice(choice):
    ...
    while choice.lower() != "r" and choice.lower() != "i" and choice.lower() != "d":
        ...
        if choice.lower() == "h":
            ...
            print(f"\n{color_words[2]}\n")
        choice = input(
            f"> Would you like to find the {color_words[3]} or {color_words[4]} or display the {color_words[16]}? Type {color_words[1]} to quit or {color_words[5]} for help. "
        )
    return choice
...
def check_price(cost):
    ...
    while not cost.isdigit():
        if cost.lower() == "q":
            ...
            print(f"\n{color_words[2]}\n")
        cost = input(
            f"> Enter the listing price of the horse in USD. Or type {color_words[1]} to quit. "
        )
    return cost
...
def intrinsic(id, cost):
    ...
    if float(results[2]) >= 0:
        print(
            "> Races Needed to Cover Cost of Horse: ",
            colored(f'{"{0:,}".format(results[2])}', "green"),
            "\n",
        )
    else:
        print(
            "> Races Needed to Cover Cost of Horse: ",
            colored(f'{"{0:,}".format(results[2])}', "red"),
            "\n",
        )
...
def run_cli():
    ...
    if id != "start-over":
        if not valid:
            ...
            if choice.lower() == "i":
                cost = input(
                    f"> Enter the {color_words[15]} of the horse in USD. Or type {color_words[1]} to quit. "
                )
...
if __name__ == "__main__":
    while True:
        run_cli()
        run_again = input(
            f"> Enter another {color_words[0]}? Type {color_words[8]} for yes or {color_words[9]} for no. "
        )
